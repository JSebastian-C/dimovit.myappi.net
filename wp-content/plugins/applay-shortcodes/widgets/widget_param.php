...
global $wl_options;
if((!$wl_options = get_option('applay')) || !is_array($wl_options) ) $wl_options = array();
...
function leafcolor_expand_control(){
	...
	update_option('applay', $wl_options);
}
...
function leafcolor_widget_add_custom_fields() {
	...
	echo "<p><label for='".esc_attr($id_disp)."-leafcolor'>".__('Custom Variation', 'applay').": <input class='widefat' type='text' name='".esc_attr($id_disp)."-leafcolor' id='".esc_attr($id_disp)."-leafcolor' value='".esc_attr($value)."' /></label></p>";
}
...
{   
    ...
    // output our extra widget logic field
    echo "
	<p id='ia-".$id_disp."'><label for='".$id_disp."-leafcolor2'>".__('Widget Style', 'applay').": 
