...
class App_Recent_Posts extends WP_Widget {	
	function __construct() {
    	$widget_ops = array(
			...
			'description' => __('App Latest Posts ','applay')
		);
    	parent::__construct('app-recent-posts', __('App - Latest Posts ','applay'), $widget_ops);
	}
	function widget($args, $instance) {
		...
		if ( isset( $cache[ $argsxx['widget_id'] ] ) ) {
			echo wp_kses_post($cache[ $argsxx['widget_id'] ]);
			return;
		}
							...
							<a href="'.esc_url(get_permalink(get_the_ID())).'" title="'.the_title_attribute('echo=0').'">
								...
								if($icon){
									if($icon_id = leafcolor_get_attachment_id_from_url($icon)){
										$thumbnail = wp_get_attachment_image_src($icon_id,'applay_thumb_80x80', true);
										$icon = isset($thumbnail[0])?$thumbnail[0]:$icon;
									} 
									$html .='<img src="'.esc_url($icon).'" width="80" height="80" alt="'.the_title_attribute('echo=0').'" title="'.the_title_attribute('echo=0').'" />';
								}else{
									$html .= get_the_post_thumbnail(get_the_ID(),'applay_thumb_80x80');
								}
								$html .='
	...
	function form( $instance ) {
		...
		?>
        <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:','applay'); ?></label>
        <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
      	<!-- /**/-->        
        <p><label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Number of posts:','applay'); ?></label>
        <input id="<?php echo esc_attr($this->get_field_id('number')); ?>" name="<?php echo esc_attr($this->get_field_name('number')); ?>" type="text" value="<?php echo esc_attr($number); ?>" size="3" /></p>

      	<!-- /**/-->
        <p>
          <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php _e('Post type','applay'); ?></label> 
          <select name="<?php echo esc_attr($this->get_field_name('post_type')); ?>" id="<?php echo esc_attr($this->get_field_id('post_type')); ?>">
              <option value="post" <?php if($post_type=='post'){?> selected="selected"<?php }?>><?php _e('Post','applay'); ?></option>
              <option value="app_portfolio" <?php if($post_type=='app_portfolio'){?> selected="selected"<?php }?>><?php _e('Portfolio','applay'); ?></option>
              <option value="product" <?php if($post_type=='product'){?> selected="selected"<?php }?>><?php _e('Product','applay'); ?></option>
            </select>
        </p>
        <p>
          <label for="<?php echo esc_attr($this->get_field_id('ids')); ?>"><?php _e('ID list show:','applay'); ?></label> 
        ...
        <p>
          <label for="<?php echo esc_attr($this->get_field_id('tags')); ?>"><?php _e('Tags:','applay'); ?></label> 
		...
		<!--//-->
         <p>
            <label for="<?php echo esc_attr($this->get_field_id('cats')); ?>" class="app-cat-widget"><?php _e('Categories:','applay');?></label>
...
function leaf_widget_init() {
    return register_widget("App_Recent_Posts");
}
